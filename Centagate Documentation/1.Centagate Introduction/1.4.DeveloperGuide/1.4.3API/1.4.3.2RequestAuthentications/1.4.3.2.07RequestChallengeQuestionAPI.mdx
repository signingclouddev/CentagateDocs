---
title: 💬Request Challenge Question API
---

# 💬Request Challenge Question API

Return list of question that need to be answer during security question authentication. The question will be return randomly if the question list is more than one.

### 🔍 URL

| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         |  [host]/v2/CentagateWS/webresources/req/kba/getQuestions                      |
...
```json
}"item": [
		{
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/req/kba/getQuestions"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	ClientResponse response = service.path ("req").path("kba").path("getQuestions").accept(MediaType.APPLICATION_JSON).post(ClientResponse.class, gson.toJson(map));
...
const options = {
	...
	port: 443,
	path:'/v2/CentagateWS/webresources/req/kba/getQuestions")',
	method: 'POST',
	headers: {
...
conn.request("POST", "/v2/CentagateWS/webresources/req/kba/getQuestions", payload, headers)
...
<?php
    ...
    $url = 'https://<domain_name>/v2/CentagateWS/webresources/req/kba/getQuestions';
