---
title: 📲Request Mobile Push CR OTP Authentication API
---
...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         | [host]/v2/CentagateWS/webresources/req/requestMobilePushCR                    |
...
| hmac           | Required    | HMAC generated from combination (username + details + authToken + userAgent + integrationKey + supportFido + browserFp + unixTimeStamp + ipAddress) using Secretkey (Can be obtained from the App page) as the key.|
...
```json
}"item": [
		{
					...
					"script": {
						"exec": [
							...
							"var unixTimestamp ='local unixtimestamp';\r"
                            				...
                            				"var message = username + details + integrationKey + unixTimestamp;\r",
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/req/requestMobilePushCR"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	String hmac = convertHmacSha256("secretkey","username" + "details" + "authToken" + "integrationKey" + "unixTimestamp" + "supportFido" + "ipAddress" + "userAgent" + "browserFp");
...
var hmacText = username + details + authToken + integrationKey + unixTimestamp + supportFido + ipAddress + userAgent + browserFp;
var hash, hmac;
...
const options = {
	...
	port: 443,
	path:'/v2/CentagateWS/webresources/req/requestMobilePushCR")',
	method: 'POST',
	headers: {
...
#calculate HMAC 
message = (USERNAME + INTEGRATIONKEY + str(UNIXTIMESTAMP) + AUTHTOKEN + SUPPORTFIDO + IPADDRESS + USERAGENT + BROWSERFP)
...
conn.request("POST", "/v2/CentagateWS/webresources/req/requestMobilePushCR", payload, headers)
...
<?php
    ...
    $url = 'https://<domain_name>/v2/CentagateWS/webresources/req/requestMobilePushCR';
    ...
    $hmac_text = $username.$details.$authToken.$integrationKey.$unixTimestamp.$supportFido.$ipAddress.$userAgent.$browserFp;
