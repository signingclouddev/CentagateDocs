---
title: Request QR Code Authentication API
---

#  Request QR Code Authentication API

This API will send a request for a QR Code for QR authentication.

### 🔍 URL

| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         | [host]/v2/CentagateWS/webresources/req/requestQrCode                          |
...
| details        | Required    | The transaction details are encoded using based64.|
...
| hmac           | Required    | HMAC generated from combination (username + devAccId + details + authToken + userAgent + integrationKey + supportFido + browserFp + unixTimestamp + ipAddress) using Secretkey (Can be obtained from the App page) as the key​. |
...
```json
}"item": [
		{
					...
					"script": {
						"exec": [
							...
							"var username ='user's username';\r",
                                                        "var devAccId ='selected device id';\r",
							...
							"var unixTimestamp ='local unixtimestamp';\r"
                            				...
                            				"var message = username + devAccId + details + integrationKey + unixTimestamp;\r",
			...
			"request": {
				...
				"url": "https://{{host}}/CentagateWS/webresources/req/requestQrCode"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	String hmac = convertHmacSha256("secretkey","username" + "devAccId" + "details" + "authToken" + "integrationKey" + "unixTimestamp" + "supportFido" + "ipAddress" + "userAgent" + "browserFp");
...
var hmacText = username + devAccId + details + authToken + integrationKey + unixTimestamp + supportFido + ipAddress + userAgent + browserFp;
var hash, hmac;
...
const options = {
	...
	port: 443,
	path:'/v2/CentagateWS/webresources/req/requestQrCode',
	method: 'POST',
	headers: {
...
#calculate HMAC 
message = (USERNAME + DEVACCID + DETAILS + AUTHTOKEN + USERAGENT + INTEGRATIONKEY + SUPPORTFIDO + BROWSERFP + str(UNIXTIMESTAMP) + IPADDRESS)
...
conn.request("POST", "/v2/CentagateWS/webresources/req/requestQrCode", payload, headers)
...
<?php
    ...
    $url = 'https://<domain_name>/v2/CentagateWS/webresources/req/requestQrCode';
    ...
    $hmac_text = $username.$devAccId.$details.$authToken.$integrationKey.$unixTimestamp.$supportFido.$ipAddress.$userAgent.$browserFp;
