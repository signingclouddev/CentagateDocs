---
title: 🧵Request Random Strings API
---

# 🧵Request Random Strings API
...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         | [host]/v2/CentagateWS/webresources/req/requestRandomString                    |
...
```json
}"item": [
		{
			...
			"request": {
				...
				"url": "https://{{host}}/CentagateWS/webresources/req/requestRandomString"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	ClientResponse response = service.path ("req").path("requestRandomString").accept(MediaType.APPLICATION_JSON).post(ClientResponse.class, gson.toJson(map));
...
const options = {
	...
	port: 443,
	path:'/v2/CentagateWS/webresources/req/requestRandomString',
	method: 'POST',
	headers: {
...
#calculate HMAC 
message = (USERNAME + AUTHTOKEN + USERAGENT + INTEGRATIONKEY + SUPPORTFIDO +
           BROWSERFP + str(UNIXTIMESTAMP) + IPADDRESS)
...
conn.request("POST", "/v2/CentagateWS/webresources/req/requestRandomString", payload, headers)
...
<?php
    ...
    $url = 'https://<domain_name>/v2/CentagateWS/webresources/req/requestRandomString';
