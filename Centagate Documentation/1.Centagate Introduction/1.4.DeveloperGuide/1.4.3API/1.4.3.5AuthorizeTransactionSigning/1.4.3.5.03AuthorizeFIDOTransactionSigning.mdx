---
title: 🛡️ Authorize FIDO Transaction Signing
---

#  🛡️ Authorize FIDO Transaction Signing
...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         | [host]/v2/CentagateWS/webresources/auth/authFidoTrans                         |
...
| fidoChallenge  | Optional    | The FIDO challenge key.     |
| assertion      | Optional    | The FIDO public key credentials.|
...
```json
}"item": [
		{
			...
			"event": [
				{
					...
					"script": {
						"exec": [
                            ...
                            "var fidoChallenge ='otp code';\r",
                            "var assertion ='generated auth token';\r",
                            ...
                            "var message = username + transactionId + details + fidoChallenge + assertion + integrationKey + unixTimestamp;\r",
                            ...
                            "postman.setEnvironmentVariable(\"fidoChallenge\", fidoChallenge);\r",
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/auth/authFidoTrans"
			},
			"response": []
		}
]
...
#calculate HMAC 
message = (USERNAME + TRANSACTIONID + DETAILS + FIDOCHALLENGE + ASSERTION + INTEGRATIONKEY + str(UNIXTIMESTAMP) + AUTHTOKEN + SUPPORTFIDO + IPADDRESS + USERAGENT + BROWSERFP)
...
conn.request("POST", "/v2/CentagateWS/webresources/auth/authFidoTrans", payload, headers)
