...
| hmac           | Required    | Hmac generated from combination (username + details + transactionId + authToken + userAgent + integrationKey + supportFido + browserFp + unixTimeStamp + ipAddress) using Secretkey (Can be obtainedfrom the App page) as the key.|
...
```json
}"item": [
		{
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/auth/requestFidoTrans"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	ClientResponse response = service.path ("req").path("requestFidoTrans").accept(MediaType.APPLICATION_JSON).post(ClientResponse.class, gson.toJson(map));
...
const options = {
	...
	port: 443,
	path:'/v2/CentagateWS/webresources/req/requestFidoTrans',
	method: 'POST',
	headers: {
...
conn.request("POST", "/v2/CentagateWS/webresources/req/requestTransactionSigningChallenge", payload, headers)
...
<?php
    ...
    $url = 'https://<domain_name>/v2/CentagateWS/webresources/req/requestFidoTrans';
