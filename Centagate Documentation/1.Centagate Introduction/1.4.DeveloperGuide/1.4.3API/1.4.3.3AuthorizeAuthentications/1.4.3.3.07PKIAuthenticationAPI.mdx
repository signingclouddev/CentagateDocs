...
| signature           | Required    | Perform Signature (SHA1WithRSA) On the Random String. The Result Is Encoded in Base64.|
...
```json
		...
		{
			"name": "/auth/authPkiWithSignature",
			"event": [
				{
					"listen": "prerequest",
					"script": {
						"exec": [
                            ...
                            "var message = username + certFingerprintSha1 + signature  + integrationkey + unixTimestamp;\r",
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/auth/authPkiWithSignature"
			},
			"response": []
		}
]
...
#calculate HMAC 
message = (USERNAME + CERTFINGERPRINTSHA1 + SIGNATURE + INTEGRATIONKEY +  str(UNIXTIMESTAMP) + AUTHTOKEN + SUPPORTFIDO + IPADDRESS + USERAGENT + BROWSERFP)
...
conn.request("POST", "/v2/CentagateWS/webresources/auth/authPkiWithSignature", payload, headers)
