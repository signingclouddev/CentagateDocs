---
title: 👤Username and Password Authentication API
---
...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         | [host]/v2/CentagateWS/webresources/auth/authBasic                             |
...
| password       | Required    | The user’s password that registered in the CENTAGATE cloud.|
			...
			"event": [
				{
					...
					"script": {
						"exec": [
							...
							"var password ='user's password';\r",
							...
							"var unixTimestamp ='local unixtimestamp';\r"
                            				...
                            				"var message = username + password + integrationKey + unixTimestamp;\r",
...
public static void main(String[] args) {
	...
	String hmac = convertHmacSha256("secretkey","username" + "password" + "integrationKey" + "unixTimestamp" + "supportFido" + "ipAddress" + "userAgent" + "browserFp");
...
var hmacText = username + password + integrationKey + unixTimestamp + supportFido + ipAddress + userAgent + browserFp;
var hash, hmac;
...
conn.request("POST", "/v2/CentagateWS/webresources/auth/authBasic", payload, headers)
...
<?php
    ...
    $url = 'https://<domain_name>/v2/CentagateWS/webresources/auth/authBasic';
    ...
    $hmac_text = $username.$password.$integrationKey.$unixTimestamp;
