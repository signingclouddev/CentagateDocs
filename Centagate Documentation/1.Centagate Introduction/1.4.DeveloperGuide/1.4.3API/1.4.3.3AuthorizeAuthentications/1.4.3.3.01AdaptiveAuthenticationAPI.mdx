---
title: 🔎Adaptive Authentication API
---
...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| POST         | [host]/v2/CentagateWS/webresources/auth/adaptive                              |
...
| authResult     | Required    | Either **"True"** or **"False"**. **True = Authentication Success**. This function will return and inform the user whether need step up authentication or not. **False = Authentication Failed**. This value send back to CENTAGATE for indexing statistic purposes.|
...
| hmac           | Required    | Hmac generated from combination (username + authResult + userAgent + integrationKey + supportFido + browserFp + unixTimestamp + authToken + ipAddress) using Secretkey (Can be obtained from the app page) as the key.|
...
```json
}"item": [
		{
			...
			"event": [
				{
					...
					"script": {
						"exec": [
							...
							"var authResult ='authentication result';\r",
							...
							"var unixTimestamp ='local unixtimestamp';\r"
                            				...
                            				"var message = username + authResult + integrationKey + unixTimestamp;\r",
							...
							"postman.setEnvironmentVariable(\"authResult\", authResult);\r",
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/auth/adaptive"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	String hmac = convertHmacSha256("secretkey","username" + "authresult(true or false)" + "integrationKey" + "unixTimestamp" + "authToken" + "supportFido" + "ipAddress" + "userAgent" + "browserFp");
...
var hmacText = username + authResult + integrationKey + unixTimestamp + authToken + supportFido + ipAddress + userAgent + browserFp;
var hash, hmac;
...
conn.request("POST", "/v2/CentagateWS/webresources/auth/adaptive", payload, headers)
