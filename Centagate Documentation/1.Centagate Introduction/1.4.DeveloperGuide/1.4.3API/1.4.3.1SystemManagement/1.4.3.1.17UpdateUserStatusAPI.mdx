---
title: 👤Update User Status API
---

# 👤Update User Status API

This function allows the admin to change the user status.

### 🔍 URL

| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| PUT          | {{host}}/CentagateWS/webresources/user/status/update/{{adminUsername}}        |
...
```json
}"item": [
		{
			...
			"event": [
				{
					...
					"script": {
						"exec": [
							...
							"var status ='token serial number';\r"
			...
			"request": {
				...
				"url": "https://{{host}}/CentagateWS/webresources/user/status/update/{{adminUsername}}"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	ClientResponse response = service.path("user").path("status").path("update").path(adminUsername).accept(MediaType.APPLICATION_JSON).put(ClientResponse.class, gson.toJson(map));
...
const options = {
	...
	port: 443,
	path:'/CentagateWS/webresources/user/status/update/'+adminUsername,
	method: 'PUT',
	headers: {
...
HOST = {{host}}
ENDPOINT = "/CentagateWS/webresources/user/status/update/" + ADMINUSERNAME
USERNAME = {{username}}
STATUS = {{status}}
...
```php
    $url = 'https://<domain_name>/CentagateWS/webresources/user/status/update/'.$adminUsername;
