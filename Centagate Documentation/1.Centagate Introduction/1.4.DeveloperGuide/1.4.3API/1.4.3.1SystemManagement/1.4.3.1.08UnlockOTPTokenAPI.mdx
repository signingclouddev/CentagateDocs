...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| PUT          | [host]/CentagateWS/webresources/token/unlockToken/[adminUsername]             |
...
```json
}"item": [
		{
			...
			"event": [
				{
					...
					"script": {
						"exec": [
							...
							"postman.setEnvironmentVariable(\"adminUsername\", tokenSn );\r",
			...
			"request": {
				...
				"url": "https://{{host}}/v2/CentagateWS/webresources/token/unlockToken/{{adminUsername}}"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	ClientResponse response = service.path("user").path("unlockToken").path(adminUsername).accept(MediaType.APPLICATION_JSON).put(ClientResponse.class, gson.toJson(map));
...
const options = {
	...
	port: 443,
	path:"/CentagateWS/webresources/token/unlockToken/"+adminUsername),
	method: 'PUT',
	headers: {
...
HOST = {{host}}
ENDPOINT = "/CentagateWS/webresources/token/unlockToken/" + ADMINUSERNAME
USERNAME = {{username}}
...
<?php
    $adminUsername = "<adminUsername>";
    $url = 'https://<domain_name>/CentagateWS/webresources/token/unlockToken/'.$adminUsername;
