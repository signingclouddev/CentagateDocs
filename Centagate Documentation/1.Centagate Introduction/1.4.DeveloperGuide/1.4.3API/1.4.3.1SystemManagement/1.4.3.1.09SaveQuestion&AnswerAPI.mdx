...
This API helps you to save user’s question and answer through API call.
...
| tokenSn        | Required    | The mobile number of the Hardware Token Serial Number. This is mandatory for token-type SMS and OTP. (For the OTP token type, make sure the token is not bound to any user and the token status is in **DEACTIVATE status**. |
| tokenType      | Required    | The token type. SMS = 1, OTP = 2. |
