...
| **METHOD**   | **PATH**                                                                      | 
| ------------ | ----------------------------------------------------------------------------- | 
| PUT          | {{host}}/CentagateWS/webresources/token/getStatus/{{adminUsername}}           |
...
```json
}"item": [
		{
			...
			"request": {
				...
				"url": "https://{{host}}/CentagateWS/webresources/token/getStatus/{{adminUsername}}"
			},
			"response": []
		}
]
...
public static void main(String[] args) {
	...
	ClientResponse response = service.path("token").path("getStatus").path(adminUsername).accept(MediaType.APPLICATION_JSON).put(ClientResponse.class, gson.toJson(map));
...
const options = {
	...
	port: 443,
	path:'/CentagateWS/webresources/token/getStatus/'+adminUsername,
	method: 'PUT',
	headers: {
...
HOST = {{host}}
ENDPOINT = "/CentagateWS/webresources/token/getStatus/" + ADMINUSERNAME
USERNAME = {{username}}
TYPE = {{type}}
...
<?php
    $adminUsername = "<adminUsername>";
    $url = 'https://<domain_name>/CentagateWS/webresources/token/getStatus/'.$adminUsername;
